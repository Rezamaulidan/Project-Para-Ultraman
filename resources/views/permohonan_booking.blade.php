...
@section('content')
                                ...
                                <tr>
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <div class="fw-bold text-dark">
                                                    {{ $booking->penyewa->nama_penyewa ?? $booking->username }}</div>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-sm btn-outline-dark rounded-pill"
                                            data-bs-toggle="modal" data-bs-target="#modalKtp{{ $booking->id_booking }}">
                                            <i class="fas fa-id-card me-1"></i> Lihat KTP
                                        </button>

                                        {{-- MODAL KTP --}}
                                        <div class="modal fade" id="modalKtp{{ $booking->id_booking }}" tabindex="-1"
                                            aria-hidden="true">
                                                    ...
                                                    <div class="modal-body text-center bg-light">
                                                        @if ($booking->penyewa->foto_ktp)
                                                            <img src="{{ asset('storage/' . $booking->penyewa->foto_ktp) }}"
                                                                class="img-fluid rounded shadow-sm" alt="Foto KTP">
                                                        @else
                                    ...
                                    {{-- 6. Tombol Aksi (Dengan SweetAlert) --}}
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">

                                            {{-- FORM TOLAK --}}
                                            <form id="form-reject-{{ $booking->id_booking }}"
                                                action="{{ route('pemilik.booking.reject', $booking->id_booking) }}"
                                                method="POST">
                                                @csrf
                                                <button type="button" class="btn btn-danger btn-sm rounded-3"
                                                    title="Tolak"
                                                    onclick="konfirmasiTolak('{{ $booking->id_booking }}', '{{ $booking->penyewa->nama_penyewa ?? 'Penyewa ini' }}')">
                                                    <i class="fas fa-times"></i>
                                                </button>
                                            </form>

                                            {{-- FORM TERIMA --}}
                                            <form id="form-accept-{{ $booking->id_booking }}"
                                                action="{{ route('pemilik.booking.approve', $booking->id_booking) }}"
                                                method="POST">
                                                @csrf
                                                <button type="button" class="btn btn-success btn-sm rounded-3 px-3 fw-bold"
                                                    title="Terima"
                                                    onclick="konfirmasiTerima('{{ $booking->id_booking }}', '{{ $booking->penyewa->nama_penyewa ?? 'Penyewa ini' }}')">
    ...
    <script>
        // 1. Fungsi Konfirmasi TERIMA
        function konfirmasiTerima(id, nama) {
            Swal.fire({
                title: 'Terima Booking?',
                text: `Apakah Anda yakin ingin menerima ${nama}? Status akan berubah menjadi DISETUJUI.`,
                icon: 'question',
        ...
        function konfirmasiTolak(id, nama) {
            Swal.fire({
                ...
                text: `Yakin ingin menolak ${nama}? Data ini akan ditandai sebagai DITOLAK.`,
                icon: 'warning',
                showCancelButton: true,
